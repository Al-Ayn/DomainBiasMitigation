...
The table matches the continuation experiment in manuscript R14.
"""
...
def residual_hessian(z, eps):
    ...
    d = 2 * N - 1
    H = mp.matrix(d, d)
    for i in range(N): H[i, i] = sum(P[i]) / eps
    for j in range(N - 1): H[N + j, N + j] = sum(P[i][j] for i in range(N)) / eps
    for i in range(N):
        for j in range(N - 1):
            H[i, N + j] = P[i][j] / eps
            H[N + j, i] = P[i][j] / eps
...
def first_dual_stage():
    Cn=np.array([[float(v) for v in row] for row in C])
    Aub=[]; bub=[]
    for i in range(N):
        for j in range(N):
            if i==j: continue
            ...
            if j<N-1: row[j]+=1
            row[-1]=1
            Aub.append(row); bub.append(Cn[i,j]-Cn[i,i])
    res=linprog(c=[0,0,0,-1],A_ub=np.array(Aub),b_ub=np.array(bub),bounds=[(None,None)]*N,method='highs')
...
def main():
    ...
    delta=first_dual_stage()
    print("first nonzero dual block exponent =",delta)
    assert abs(delta-0.3930843911178359)<1e-12
    print("\nTARGET TABLE")
    print("eps      warm error        extrapolated error   Newton warm  Newton extrap")
    rows = []
    for eps in eps_grid[2:]:
        true = solutions[str(eps)]; warm = solutions[str(2 * eps)]; old = solutions[str(4 * eps)]
        pred = mp.matrix([mp.mpf("1.5") * warm[i] - mp.mpf("0.5") * old[i] for i in range(len(warm))])
        warm_err = inf_norm(warm, true); pred_err = inf_norm(pred, true)
        _, iw, _ = solve(eps, warm); _, ip, _ = solve(eps, pred)
        rows.append((eps, warm_err, pred_err, iw, ip))
        print(f"{float(eps):0.4f}  {mp.nstr(warm_err, 11):>15}  {mp.nstr(pred_err, 11):>18}  {iw:11d}  {ip:13d}")
    expected = [
    ...
    E=np.array([float(r[0]) for r in rows]); pe=np.array([float(r[2]) for r in rows]); fits=[]
    for start in [0,1,2]:
        Xr=np.c_[np.ones(len(E[start:])), -1/(4*E[start:])]
        yy=np.log(pe[start:]/E[start:])
        fits.append(float(np.linalg.lstsq(Xr,yy,rcond=None)[0][1]))
    print("fitted block exponents (all,last3,last2) =",fits)
    assert abs(fits[0]-0.3459743151)<1e-8
    assert abs(fits[1]-0.3745248013)<1e-8
    assert abs(fits[2]-0.3891710738)<1e-8
